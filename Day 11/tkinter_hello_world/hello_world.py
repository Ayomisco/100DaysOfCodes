...
def submit_name():
    ...
    if case_style.get() == 'normal':
        name_label = tkinter.Label(output_frame, text="Hello " + name.get() + "! Keep 
        learning Tkinter!", bg=output_color)
    elif case_style.get() == 'upper':
        name_label = tkinter.Label(output_frame, text=("Hello " + name.get() + "! Keep 
        learning Tkinter!").upper(), bg=output_color)
...
value='upper', bg=input_color) normal_button.grid(row=1, column=0, padx=2, pady=2)
